...
)
{
	...
	#pragma unroll
	for(int i = 0; i < dim; i++)
		tmp_cords[i] = lower_bounds[i] + curand_uniform(&state) *
			(upper_bounds[i] - lower_bounds[i]);
		
	tmp_value = optimization_function(&sh_cords[threadIdx.x], dim);
	if(tmp_value < sh_values[threadIdx.x])
	{
		sh_values[threadIdx.x] = tmp_value;
		#pragma unroll
		for(int i = 0; i < dim; i++)
			sh_cords[threadIdx.x + i] = tmp_cords[i];
	}
	int trials = 0;
	...
	for(int i = 0; i < max_generations; i++)
	{
		...
		for(int i = 0; i < dim; i++)
		{
			...
			tmp_cords[i] = (tmp_cords[i] < upper_bounds[i]) * tmp_cords[i] +
				           !(tmp_cords[i] < upper_bounds[i]) * upper_bounds[i];
			//tmp_cords[i] = fast_clip_float(
			//	tmp_cords[i],
			//	lower_bounds[i],
			//	upper_bounds[i]
...
)
{
	...
	size_t hive_cords_size = DIMENSIONS * COLONY_POOL *
		                     GRID_SIZE * num_of_bees*sizeof(float);
