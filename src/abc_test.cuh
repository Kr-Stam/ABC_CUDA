...
)
{
	...
	{ // create out_path
		...
		if (selection_type == ROULETTE_WHEEL || selection_type == RANK)
		{
			std::string sort_str = SortingCpuToString(sorting_type).data();
			out_path.append("/");
			out_path.append(sort_str);
			ensure_created(out_path);
			out_path.append("/");
		}
	...
	opt_func optimization_function = GetCpuTestFunctionPtr(test_function);
	cpu::init_bees<dim>(
		&bees,
		 num_of_bees,
	...
	float* cords  = (float*) malloc(dim*num_of_bees*sizeof(float));
	...
	for(int i = 0; i < num_of_steps; i++)
	{
		...
		float error = calc_error(global_min_value, min_bee.value);
		float param_convergence = calc_param_convergence<dim>(
			global_min_params,
			min_bee.coordinates
		);
		float grad_norm = calc_grad_norm(min_bee.value, prev_min_value); 
		std::fprintf(
			file,
			"%ld;%d;%f;%f;%f;%f\n",
			duration,
	...
	if(debug)
	{
		...
		std::vector<float> coordinates_x(num_of_bees);
		std::vector<float> coordinates_y(num_of_bees);
...
public:
	...
	static constexpr std::array<uint32_t, 1> num_of_beess = {1000};
...
class RouletteCpuTest : private CpuTestBase
{
	...
	static constexpr std::array<SortingCpu, 2> sort_vals = {FULL_SORT, PARTIAL_SORT};
...
public:
	void for_all_combinations(
		...
		for_each_index(std::make_index_sequence<sort_vals.size()>(), [&](auto i_sort) {
		for_each_index(std::make_index_sequence<num_of_beess.size()>(), [&](auto i_num_of_bees) {
		for_each_index(std::make_index_sequence<step_vals.size()>(), [&](auto i_step) {
			constexpr uint32_t trial = trials_vals[i_trials];
			constexpr Roulette roul = roul_vals[i_roul];
			constexpr SortingCpu roul_sort = sort_vals[i_sort];
			constexpr uint32_t num_of_bees = num_of_beess[i_num_of_bees];
			constexpr uint32_t step = step_vals[i_step];

			abc_cpu_test<
				dim, num_of_bees, num_of_bees/10, trial, ROULETTE_WHEEL, roul,
...
class RankCpuTest : private CpuTestBase
{
	...
	static constexpr std::array<SortingCpu, 2> sort_vals = {
		FULL_SORT,
		PARTIAL_SORT,
	};
...
public:
	void for_all_combinations(
		...
		for_each_index(std::make_index_sequence<step_vals.size()>(), [&](auto i_step) {
			constexpr uint32_t trial = trials_vals[i_trials];
			constexpr Rank rank = rank_vals[i_rank];
			constexpr SortingCpu sorting = sort_vals[i_sort];
			constexpr uint32_t num_of_bees = num_of_beess[i_num_of_bees];
			constexpr uint32_t step = step_vals[i_step];

			abc_cpu_test<
				dim, num_of_bees, num_of_bees/10, trial, RANK, SUM, sorting, rank,
...
public:
	void for_all_combinations(
		...
		for_each_index(std::make_index_sequence<step_vals.size()>(), [&](auto i_step) {
			...
			abc_cpu_test<
				dim, num_of_bees, num_of_bees/10, trial, TOURNAMENT, SUM, FULL_SORT, LINEAR_ARRAY, SINGLE,
...
public:
	void for_all_combinations(
		...
		for_each_index(std::make_index_sequence<step_vals.size()>(), [&](auto i_step) {
			...
			abc_cpu_test<
				dim, num_of_bees, num_of_bees/10, trial, TOURNAMENT, SUM, FULL_SORT,
