...
float problems::cpu::easom(float* args, int n)
{
	if(n < 2) return 0;
...
float problems::cpu::michalewicz(float* args, int n, int m)
{
	float result = 0;

	for(int i = 0; i < n; i++)
		result = result - std::sin(args[i]) *
			     std::pow(std::sin((i+1)*args[i]*args[i]/M_PI), 2*m);

	return result;
}

float problems::cpu::michalewicz2(float* args, int n)
{
