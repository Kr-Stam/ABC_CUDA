...
__device__ float problems::gpu::ackley(float* args, int n, float a, float b, float c)
{
	float result;
	float sum_x2 = 0;
	float sum_cx = 0;
...
__device__ float problems::gpu::ackley2(float* args, int n)
{
	return ackley(args, n, 20, 0.2, 2*M_PI);
}

///\note this function is only a 2d function
__device__ float problems::gpu::bukin6(float* args, int n)
{
	if(n < 2) return 0;
...
///\note this function in only a 2d function
__device__ float problems::gpu::drop_wave(float* args, int n)
{
	if(n < 2) return 0;
...
///\note this function in only a 2d function
__device__ float problems::gpu::eggholder(float* args, int n)
{
	if(n < 2) return 0;
...
__device__ float problems::gpu::gramacy_and_lee(float* args, int n)
{
	if(n < 1) return 0;
...
__device__ float problems::gpu::griewank(float* args, int n)
{
	float result  = 0;
	float product = 1;
	for(int i = 0; i < n; i++)
...
__device__ float problems::gpu::holder_table(float* args, int n)
{
	if(n < 2) return 0;
...
__device__ float problems::gpu::langerman(float* args, int n, float* c, int m, float* A)
{
	float result = 0;
	for(int i = 0; i < m; i++)
	{
...
__device__ float problems::gpu::langerman2(float* args, int n)
{
	float A[] = {
		3, 5,
		5, 2,
...
__device__ float problems::gpu::levy(float* args, int n)
{
	float tmp, tmp1;
	float result = 0;
...
__device__ float problems::gpu::levy13(float* args, int n)
{
	if(n < 2) return 0;
...
__device__ float problems::gpu::rastrigin(int a, float* x, int n)
{
	float result = a * n;
	for(int i = 0; i < n; i++)
	{
...
///      definition
__device__ float problems::gpu::rastrigin2(float* args, int n)
{
	return rastrigin(10, args, n);
}

///\note This is only a 2d function
__device__ float problems::gpu::schaffer2(float* args, int n)
{
	if(n < 2) return 0;
...
///\note This is only a 2d function
__device__ float problems::gpu::schaffer4(float* args, int n)
{
	if(n < 2) return 0;
...
__device__ float problems::gpu::schwefel(float* args, int n)
{
...
///\note This is only a 2d function
__device__ float problems::gpu::shubert(float* args, int n)
{
	if(n < 2) return 0;
	float sum1 = 0;
	float sum2 = 0;
